...
export async function GET() {
  try {
    ...
    const [
      ...
      getCount(supabaseAdmin.from("v_member_status").select("*", { count: "exact", head: true }).eq("display_status", "active")),
      getCount(supabaseAdmin.from("v_member_status").select("*", { count: "exact", head: true }).eq("display_status", "frozen")),
      getCount(
        supabaseAdmin
      ...
      supabaseAdmin.from("v_member_status").select("plan_name").eq("display_status", "active"),
      supabaseAdmin
        .from("v_member_status")
        .select("*")
